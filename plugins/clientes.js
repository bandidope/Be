...
const ClienteSchema = new mongoose.Schema({
  telefono: { type: String, unique: true }, // <-- El único unique
  nombre: String,
  grupos: { type: [String], default: [] },
  fechaCompra: { type: Date, default: null }
})

const Cliente = mongoose.model('Cliente', ClienteSchema) // Sin jid
...
let handler = async (m, { command, text = '' }) => {
  if(command==='addcliente'){
    ...
    await Cliente.updateOne({telefono:t},{nombre:n,grupos,fechaCompra:fecha},{upsert:true});
